...
int loop() {
	...
	while(1) {
		...
		if (args[0] == "echo" && args[1] == "*") {
			args[1] = "**"; 
		}
		
		args = Ways(args);
		if (args[0] == args[1]) args.erase(args.begin());
		//Execution(args);
		// for(auto i:args){
		// 	cout<<i<<endl;
		// }
		if (args[0] == "cd") {
			if(args.size() == 1){
				...
				DIR *curDir_dir;
				curDir_dir = opendir(args[1].c_str());
                if (chdir(args[1].c_str()) < 0) perror("chdir");
				closedir(curDir_dir);
            }
		...
		if (pid == 0) {
			if ((args.size() > 0) && (args[0] == "time")) {
				...
				if (newpid == 0) {
					...
					long long int sys_sec = sys_end.tv_sec - sys_start.tv_sec;
					long long int sys_usec = (sys_end.tv_usec - sys_start.tv_usec) / 1000;
					long long int u_sec = u_end.tv_sec - u_start.tv_sec;
					long long int u_usec = (u_end.tv_usec - u_start.tv_usec) / 1000;
					long long int real_sec = real_end.tv_sec - real_start.tv_sec;
					long long int real_usec = (real_end.tv_usec - real_start.tv_usec) / 1000;
					cout << endl;
					printf("real	  [%lld.%llds]\n", real_sec, real_usec);
					printf("user      [%lld.%llds]\n", u_sec, u_usec);
					printf("sys       [%lld.%llds]\n", sys_sec, sys_usec);
				}
				exit(0);
			}
...
void InOutStreams(vector<string> &args) {
	...
	while (i < args.size()) { 
		...
		if ((args[i].find('<') != args[i].npos) || 
			...
			close(stoi(substr1));
			//cout<< '_' << substr1 << '_' << substr2 << '_' << endl;
